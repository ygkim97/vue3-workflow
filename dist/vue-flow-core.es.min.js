...
function useFlowCommon() {
  ...
  const onNodeDragStart = ({ nodes: nodes2 }) => {
    nodes2.forEach(({ id: id2 }) => {
      const { position } = { ...findNode(id2) };
      dragStartPositionMap.value[id2] = { x: position.x, y: position.y };
    });
  };
  ...
  const resetNodePositionIfOccupied = (nodes2) => {
    ...
    const originNodePositionList = nodes2.map(({ id: id2, position }) => {
      ...
      const originPosition = { position: dragStartPositionMap.value[id2] };
